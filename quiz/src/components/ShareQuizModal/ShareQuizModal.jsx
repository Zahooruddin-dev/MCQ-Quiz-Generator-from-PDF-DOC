...
const ShareQuizModal = ({ 
  ...
  const [shareUrl] = useState(`${window.location.origin}/shared-quiz/${Date.now()}`);
  ...
  const handleSocialShare = (platform) => {
    const text = shareMessage || `I just scored ${userResults?.score}% on "${quizData?.title}" quiz! 🎯`;
    const url = shareUrl;
          ...
          <Stack spacing={3}>
            ...
            <Box>
              ...
              <Stack direction="row" spacing={1} sx={{ mb: 2 }}>
                <Chip label={`${quizData?.totalQuestions || 0} Questions`} size="small" variant="outlined" />
                <Chip label={quizData?.difficulty || 'Medium'} size="small" variant="outlined" />
